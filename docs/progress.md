...
- **里程碑**：M2 进行中 —— **M2-a 采购单据、M2-b 到货→入库→库存流水、M2-c 出库/调拨/盘点 + 红冲已完成**；下一步 M2-d 库存预警/日快照/供货价
- **更新时间**：2026-09-18
...
**M2-d：库存预警 / 日结存快照 / 供货价（后端）** —— 落地 `docs/db-schema.md` §11.3–§11.5：
新增 `stock_alert`、`inventory_snapshot_daily`、`material_supplier_price` ORM/迁移；
实现低库存/超储/临期预警扫描（依据 `material.safety_stock`/`max_stock` 与批次 `expiry_date`，同物料/仓库/批次/类型未关闭不重复），
每日结存快照 upsert（粒度物资×仓库×日，供预测聚合），以及供货价维护（供采购买单取价）；
补预警/对账测试并扩展 `scripts/check_invariants.py`。

## 已知坑 / 未决问题

| 项 | 说明 | 状态 |
|---|---|---|
| 运行库实例 | 已起独立 PG16 容器（`deploy/docker-compose.yml`，仅 127.0.0.1:5433）。真库验证：迁移 upgrade/downgrade 可逆、`jsonb`、`postgresql_where` 部分唯一索引、`COALESCE` 表达式唯一索引；测试支持 `ERP_TEST_DATABASE_URL` 对 PG 跑（43 passed） | 已解决（2026-09-18） |
...
| 领域不变量脚本 | `scripts/check_invariants.py`：状态机结构 + 权限码 + service 不直改 status + 库存只由 `service/stock_ledger.py` 过账且必须写流水（verify 用 venv python 跑） | M2-b/M2-c 完成 |
