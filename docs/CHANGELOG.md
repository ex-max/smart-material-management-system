...
## 2026-09-19 · 现实演示数据入库（M8）

- **改动**：新增 `backend/scripts/seed_demo.py`（仅标准库，通过 HTTP API 造数）与 `Makefile` 的 `seed-demo` 目标。写入现实数据：单位 10 / 分类 21（两级树）/ 供应商 8 / 仓库 3 / 库位 10 / 物资 26；完整走通「请购→审批→转采购订单→到货→验收入库」，含批次物资（清洁剂，带批次号与效期）、出库（生产领料/报废）、调拨（跨仓备件）、盘点（含盘盈/盘亏）、库存预警扫描；单据状态覆盖草稿/待审/已审/执行中/完成。
- **原因**：用户要求向数据库添加符合逻辑、符合现实的演示数据。选择走 API 而非直接写库：单据必须走状态机、库存结存只能由 `stock_ledger` 流水推导（AGENTS 第五节）。
- **依赖**：无（仅标准库）。
- **验证**：脚本执行成功；总数见上；`GET /inventory/reconcile` → **ok=true**（四项差异均为 0）；库存结存 10 行、库存流水 17 条。`make verify` 保持绿（本改动仅新增脚本/Makefile/文档）。
