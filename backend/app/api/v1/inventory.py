...
# ---------------- 库存查询 ----------------
@router.get("/inventory", name="list_inventory")
def list_inventory(
...
@router.get("/inventory/batches", name="list_inventory_batches")
def list_batches(
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=200),
    material_id: int | None = Query(None),
...
@router.get("/inventory/transactions", name="list_inventory_transactions")
...
@router.get("/inbound-orders/{inbound_id}", name="get_inbound_order")
...
@router.post("/inbound-orders/{inbound_id}/post", name="post_inbound_order")
